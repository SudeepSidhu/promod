...
const VERSION_NAME = "promod_v1rc7_b3";
...
function ModifyVehicles()
{
	...
	foreach AllActors(class'Gameplay.VehicleSpawnPoint', vehiclePad)
	{
		if(EnablePod)
		{
			...
			vehiclePad.vehicleClass = class'promod_v1rc7_b3.promodPod';
		}
		else if(!EnablePod)
		{
			if (vehiclePad != None && vehiclePad.vehicleClass == class'VehicleClasses.VehiclePod')
		...
		if(EnableRover)
		{
			...
			{				
				if(EnableRoverGun)
				{
					vehiclePad.vehicleClass = class'promod_v1rc7_b3.promodDefaultBuggy';
				} else {
					vehiclePad.vehicleClass = class'promod_v1rc7_b3.promodBuggy';
				}
			}
			else if(!EnableRover)
			{
		...
		if(EnableTank)
		{
			...
			vehiclePad.vehicleClass = class'promod_v1rc7_b3.promodTank';
		}
		else if(!EnableTank)
		{
			if (vehiclePad != None && vehiclePad.vehicleClass == class'VehicleClasses.VehicleTank')
...
function ModifyStats()
{
	...
	if(M != None && BonusStatsOn && !TournamentOn() && CTFOn())
	{
		...
		// Head Shot
		M.projectileDamageStats[statCount].damageTypeClass = Class'promod_v1rc7_b3.promodSniperProjectileDamageType';
		...
		// E-Blade
		M.extendedProjectileDamageStats[statCount].damageTypeClass = Class'promod_v1rc7_b3.promodBladeProjectileDamageType';
...
function Actor ReplaceActor(Actor Other)
{
	...
	{
		Other.Destroy();
		return ReplaceWith(Other, "promod_v1rc7_b3.promodWeaponEnergyBlade");
		return Super.ReplaceActor(Other);
	}
	...
	else if(Other.IsA('WeaponVehicleTank'))
	{
		if(TankUsesMortarProjectile == true)
		{
			...
			WeaponVehicleTank(Other).projectileClass = Class'promod_v1rc7_b3.promodTankRound';
...
function string MutateSpawnCombatRoleClass(Character c)
{
	...
	c.team().combatRoleData[0].role.default.armorClass.default.AllowedWeapons = class<Armor>(DynamicLoadObject("promod_v1rc7_b3.promodArmorLight", class'class')).default.AllowedWeapons;
	c.team().combatRoleData[1].role.default.armorClass.default.AllowedWeapons = class<Armor>(DynamicLoadObject("promod_v1rc7_b3.promodArmorMedium", class'class')).default.AllowedWeapons;
	c.team().combatRoleData[2].role.default.armorClass.default.AllowedWeapons = class<Armor>(DynamicLoadObject("promod_v1rc7_b3.promodArmorHeavy", class'class')).default.AllowedWeapons;
