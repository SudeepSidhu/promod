...
const VERSION_NAME = "promod_v1rc7_b1";

/**
*
* promod's shizzle test
*/
...
function ModifyVehicles()
{
	...
	foreach AllActors(class'Gameplay.VehicleSpawnPoint', vehiclePad)
	{
		if(EnablePod)
		{
			...
			vehiclePad.vehicleClass = class'promod_v1rc7_b1.promodPod';
		}
		else if(!EnablePod)
		{
			if (vehiclePad != None && vehiclePad.vehicleClass == class'VehicleClasses.VehiclePod')
		...
		if(EnableRover)
		{
			...
			{				
				if(EnableRoverGun)
				{
					vehiclePad.vehicleClass = class'promod_v1rc7_b1.promodDefaultBuggy';
				} else {
					vehiclePad.vehicleClass = class'promod_v1rc7_b1.promodBuggy';
				}
			}
			else if(!EnableRover)
			{
		...
		if(EnableTank)
		{
			...
			vehiclePad.vehicleClass = class'promod_v1rc7_b1.promodTank';
		}
		else if(!EnableTank)
		{
			if (vehiclePad != None && vehiclePad.vehicleClass == class'VehicleClasses.VehicleTank')
...
function ModifyStats()
{
	...
	if(M != None && BonusStatsOn && !TournamentOn() && CTFOn())
	{
		...
		// Head Shot
		M.projectileDamageStats[statCount].damageTypeClass = Class'promod_v1rc7_b1.promodSniperProjectileDamageType';
		...
		// SniperRifle HS
		// M.extendedProjectileDamageStats[statCount].damageTypeClass = Class'promod_v1rc7_b1.promodSniperProjectileDamageType';
		// ++statCount;
		
		// E-Blade
		M.extendedProjectileDamageStats[statCount].damageTypeClass = Class'promod_v1rc7_b1.promodBladeProjectileDamageType';
		...
		M.extendedProjectileDamageStats[statCount].extendedStatClass = Class'statOneEightSeven';
		++statCount;
...
function Actor ReplaceActor(Actor Other)
{
	...
	else if(Other.IsA('WeaponHandGrenade'))
	{
	...
	else if(Other.IsA('WeaponMortar'))
	{
		WeaponMortar(Other).projectileClass = Class'promodProjectileMortar';
		return Super.ReplaceActor(Other);
	}
	else if(Other.IsA('WeaponGrenadeLauncher'))
	{
	...
	{
		Other.Destroy();
		return ReplaceWith(Other, "promod_v1rc7_b1.promodWeaponEnergyBlade");
		return Super.ReplaceActor(Other);
	}
	else if(Other.IsA('WeaponBurner'))
	{
	...
	else if(Other.IsA('WeaponVehicleTank'))
	{
		if(TankUsesMortarProjectile == true)
		{
			...
			WeaponVehicleTank(Other).projectileClass = Class'promod_v1rc7_b1.promodTankRound';
	...
	else if(Other.IsA('InventoryStationAccess'))
	{
	...
	else if(Other.IsA('CatapultDeployable'))
	{
		CatapultDeployable(Other).basedeviceClass = Class'promodDeployedCatapult';
		return Super.ReplaceActor(Other);
	}
	else if(Other.IsA('InventoryStationDeployable'))
	{
	...
	else if(Other.IsA('CloakPack'))
	{
		Other.Destroy();
		return ReplaceWith(Other, VERSION_NAME $ ".promodanticloak");
	}
	else if(Other.IsA('EnergyPack'))
	{
...
function string MutateSpawnCombatRoleClass(Character c)
{
	...
	c.team().combatRoleData[0].role.default.armorClass.default.AllowedWeapons = class<Armor>(DynamicLoadObject("promod_v1rc7_b1.promodArmorLight", class'class')).default.AllowedWeapons;
	c.team().combatRoleData[1].role.default.armorClass.default.AllowedWeapons = class<Armor>(DynamicLoadObject("promod_v1rc7_b1.promodArmorMedium", class'class')).default.AllowedWeapons;
	c.team().combatRoleData[2].role.default.armorClass.default.AllowedWeapons = class<Armor>(DynamicLoadObject("promod_v1rc7_b1.promodArmorHeavy", class'class')).default.AllowedWeapons;
